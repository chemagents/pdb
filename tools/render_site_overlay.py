...
def project_box_edges(box: dict, plane: str) -> list[tuple[float, float]]:
...
def build_panel_svg(
    ...
    combined = projected_reference + projected_ligands + project_box_edges(box, plane)
    ...
    scaled_box = scale_with_bounds(project_box_edges(box, plane), bounds, PANEL_WIDTH, PANEL_HEIGHT, MARGIN)
    ...
    box_d = " ".join(
        [f"M {scaled_box[0][0]:.2f} {scaled_box[0][1]:.2f}"]
        + [f"L {point[0]:.2f} {point[1]:.2f}" for point in scaled_box[1:]]
    )
    ...
    return (
        ...
        f'<path d="{box_d}" stroke="#2563eb" stroke-width="1.6" fill="none" stroke-dasharray="6 4" />'
        + "".join(circles)
        + '</g>'
    )


def build_multi_panel_svg(reference_points: list[tuple[float, float, float]], ligand_entries: list[dict], box: dict, title_prefix: str) -> str:
    panels = []
    for index, plane in enumerate(["xy", "xz", "yz"]):
        panel_svg = build_panel_svg(plane, reference_points, ligand_entries, box, f"{title_prefix} ({plane.upper()})")
...
def build_html_review(reference_protomer: dict, final_box: dict, primary_entries: list[dict], outlier_entries: list[dict]) -> str:
    return f"""<!DOCTYPE html>
<html lang=\"en\">
<head>
  <meta charset=\"utf-8\" />
  <title>Site Overlay Review: {reference_protomer['gene_name']}</title>
  <style>
...
<body>
  <h1>Site Overlay Review: {reference_protomer['gene_name']}</h1>
  <div class=\"meta\">
  ...
  <div class=\"card\">
    ...
    <p>Reference backbone in gray, aligned ligand atoms in red, final box in blue dashed outline.</p>
    <img src=\"overlay_all_accepted.svg\" alt=\"Accepted overlay\" />
  </div>
  <div class=\"card\">
    ...
    <img src=\"overlay_outliers.svg\" alt=\"Outlier overlay\" />
  </div>
  <div class=\"card\">
    <h2>Final Box Overlay</h2>
    <p>Same reference frame, focused on the primary site cluster used for docking box construction.</p>
    <img src=\"final_box_overlay.svg\" alt=\"Final box overlay\" />
  </div>
</body>
</html>
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Render SVG/HTML visualization artifacts for site overlay review."
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "04_sites/, 05_box/ and visuals are written into output/<gene_name>/06_visuals/."
        ),
    )
    ...
    try:
        ...
        visuals_dir = Path(args.output_dir) / gene_name / "06_visuals"
        manifests_dir = visuals_dir / "manifests"
        visuals_dir.mkdir(parents=True, exist_ok=True)
        manifests_dir.mkdir(parents=True, exist_ok=True)

        accepted_svg = build_multi_panel_svg(reference_points, primary_entries, final_box_qc, "Accepted overlay")
        outlier_svg = build_multi_panel_svg(reference_points, outlier_entries, final_box_qc, "Outlier overlay")
        box_svg = build_multi_panel_svg(reference_points, primary_entries, final_box_qc, "Final box overlay")

        write_text(visuals_dir / "overlay_all_accepted.svg", accepted_svg)
        write_text(visuals_dir / "overlay_outliers.svg", outlier_svg)
        write_text(visuals_dir / "final_box_overlay.svg", box_svg)
        write_text(visuals_dir / "overlay_review.html", build_html_review(reference_protomer, final_box, primary_entries, outlier_entries))

        visuals_manifest = {
            "gene_name": gene_name,
            "pipeline_stage": 6,
            "stage_name": "visualization_and_review",
            "reference_protomer_instance_id": reference_protomer["protomer_instance_id"],
            "accepted_overlay_file": f"output/{gene_name}/06_visuals/overlay_all_accepted.svg",
            "outlier_overlay_file": f"output/{gene_name}/06_visuals/overlay_outliers.svg",
            "final_box_overlay_file": f"output/{gene_name}/06_visuals/final_box_overlay.svg",
            "html_review_file": f"output/{gene_name}/06_visuals/overlay_review.html",
            "primary_overlay_ligand_count": len(primary_entries),
            "outlier_overlay_ligand_count": len(outlier_entries),
        }
        write_json(visuals_manifest, manifests_dir / "visuals.json")

        report_lines = [
            f"# Visual Review Summary: {gene_name}",
            "",
            f"- Reference protomer: `{reference_protomer['protomer_instance_id']}`",
            f"- Primary aligned ligands shown: {len(primary_entries)}",
            f"- Outliers shown: {len(outlier_entries)}",
            f"- HTML review: `output/{gene_name}/06_visuals/overlay_review.html`",
            f"- Accepted overlay: `output/{gene_name}/06_visuals/overlay_all_accepted.svg`",
            f"- Final box overlay: `output/{gene_name}/06_visuals/final_box_overlay.svg`",
        ]
        write_text(visuals_dir / "visual_summary.md", "\n".join(report_lines) + "\n")

        print(visuals_dir / "overlay_review.html")
        return 0
    except Exception as exc:  # noqa: BLE001
        print(f"Failed to render site overlays for {args.gene_name}: {exc}", file=sys.stderr)
        return 1
